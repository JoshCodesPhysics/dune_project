...
The FIR algorithm acts as a random noise filter for the pedestal-subtracted ADC values, so that the final signal entering the hit finder block is clean enough not to generate false hits. Figure \ref{fig:pedsubfirblockdiagram} displays how the \texttt{pedsub\_HLS\_SSR} and \texttt{fir\_HLS\_SSR} ports connect. Included in figure \ref{fig:firanaloguewaveform} is a visual representation of the effect that the low-pass, 32-tap filter has on the incoming ADC values. From this figure one can see how the hits can be more easily identified by the hit finder block, since the peaks are more distinct and the time spent above a certain threshold value can be more accurately measured.
...
Given more time to investigate the \texttt{axis} interface directives detailed in section \ref{axi4_method}, insight could have been provided into the resource usage of pipelined HLS-designed blocks with the same latency as the manual HDL designs. In this case, the efficiency of high-level synthesised implementations could be brought into question; section \ref{Discussion} describes studies done involving this inquiry.
...
The pursuit of using HLS to redesign hit finder architecture in the ProtoDUNE II detector has led to a number of discoveries. Conclusions can subsequently be made regarding the accessibility of the software, the performance and efficiency of the generated IP files, and the viability for experimental physicists looking to generate firmware in the future.
...
In this report, the theoretical and experimental motivations for the DUNE experiment have been outlined. Details of the hit finder architecture and the HF algorithms were discussed, and the original VHDL designs were compared to the new HLS implementations. The following segment summarises the discoveries made during the progression of the project.
...
The following section shall summarise the experience of designing hit finder algorithms with the Vivado HLS software.
