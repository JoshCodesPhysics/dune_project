...
On input, the first logic branch involves the \texttt{tready} input being high or low. If \texttt{tready} is low i.e. back pressure is active and the block operation requires pausing, the mechanism will force an output of the same values sent through 1 clock cycle before the back pressure went active. This can be achieved by constantly saving the previous clock's values and then restoring them to the output once \texttt{tready} input goes low. However, if the block latency was greater than 1 clock cycle and the II $= 1$, back pressure would occur halfway through the operation of one function call, meaning the block would not react in time and erroneous outputs would be sent through to the FIR.
...
% Outdated statistics analysis. Change this and then we can start working on the rest of the technical section. Once that is finished, go for the theoretical side. Finally, write the big picture/contextualise sections.

When comparing resource utilisation between the \texttt{pedsub} blocks, detailed in figures \ref{fig:pedsubblockstats} and \ref{fig:hlsblockstats}, the most important components to compare are LUTs, input and output (IO)\footnote{These are physical structures that allow the connection of an FPGA target to other devices within the system.} resources for the FPGA target\footnote{A programmable area or chip on the FPGA used to implement a digital circuit, in our case designed via HLS or directly through VHDL.}, and FFs.

The utilization tables show that the new design requires $0.06\%$ more LUT usage when compared to the total LUTs available, which although comes to a $274\%$ increase in total LUT usage for the block, is negligible when compared to the total available. Similarly, the FF usage for the HLS design shows an $140\%$ increase from the original, but only a $0.02\%$ difference in total usage of all available FFs. The disparity in LUT and FF usage can be explained by the additional internal logic and registers required to implement the delayed tlast restore mechanism and the instantaneous response to tready.

However, the difference in IO resource usage is not negligible, due to the large number of scalar and pointer type ports in the \texttt{pedsub\_HLS} design compared to the original. There is a $25\%$ increase in the FPGA target IO usage of the new design when compared to the total available, and a $195\%$ total increase in the number of IO resources used. This can be remedied by removing unnecessary ports and incorporating multiple ports as members of a C++ \texttt{struct} type input, which can be manipulated further to reduce IO usage. This method is used in the designs featured in section \ref{pedsub_w_SSR}, \ref{fir_ext_SSR} and \ref{FIR_SSR}.

\subsubsection{Pedestal Subtraction with Implemented SSR} \label{pedsub_w_SSR}

Look at HLS code and describe how I did it. What did I find out about this since the prelim report? Next section talks about the discovery of external arrays automatically implemented as BRAMs for long term value storage. These are power on initializations (like static variables) but actually are not wiped in between packets as previously thought?

\subsubsection*{BRAM Utilisation for the SSR Mechanism} \label{BRAM_SSR}

One read and/or write per clock, this is ok though for accumulator and pedestal restore. This may not need to be it's own section but important to discuss.
