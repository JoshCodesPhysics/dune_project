...
The LAr TPC modules used in the ProtoDUNE II design contain 150 anode plane assemblies (APA's) with 2560 wires per APA. There are 10 fibres/data streams in the APA, each with 256 wires - hence the total of 2560. The Front End LInk eXchange system is used to receive the data from the APAs, and was first developed within the ATLAS collaboration. It is based on custom FPGA-based PCIe I/O cards, combined with commodity servers \cite{borga2019felix}. Within this system contains the focus of this project: the Trigger Primitive Generation (TPG) core, which will be discussed in more detail later.

The readout window per event is between 3-5.5 ms for a 2.25 ms drift time.
The FELIX architecture that handles this influx of events consists of 10 MGT modules, whose function is to process the APA data stream into computational bits, that feed data into 10 Hit Finders (HF). The hits are produced by processing the incoming ADC (Analog-to-Digital Converted) samples in parallel. The MGT output enters the HFs via the data router\footnote{The data router receives multiple-channel, single clock-tick packets. The packets initially exist in a format containing the first sample for all channels, and when processed by the data router are converted into a format subsisting of all the samples for a given channel i.e. multiple clock ticks of data.} and is then transferred into 10 sets of 4 TPG block processors; one set for each MGT output. These are multiplexed\footnote{Combined into a singular signal output} by the arbitrator into the Central Router (CR) interface \cite{abi_barr_martin-albo_azfar_2017, adams2020protodune}. See figure \ref{fig:hfprocessor} for a visualisation of the packet's journey from the MGT output to the CR, and figure \ref{fig:fearch} for the dataflow overview.

\subsection{The TPG Core}

Each TPG unit processes the data from 64 APA wires, and so there are 4 of them for each core\footnote{$4\times 64 = 256$, the number of wires in a fiber, ten fibers for each MGT accounts for all wires within the APA}.
...
There is an AXI4-stream protocol for unpacking and reordering of data in the TPG block, featuring 4 general signals for various instructions in each component:
...
\begin{enumerate}
	...
	\item Technology life-cycle (TLC) of the components. We need long-lasting essential components such as the FPGA and network components.
