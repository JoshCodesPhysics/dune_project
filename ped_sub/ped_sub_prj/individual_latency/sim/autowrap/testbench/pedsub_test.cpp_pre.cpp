...
# 27 "/data/hmo31799/ped_sub/pedsub_test.cpp"
 ...
 set_rnd_seed((800 + 400)/2, rand_seed);


 packet_loop: for (i = 0; i < 5; i++) {
  channel_loop: for (j = 0; j < 10; j++) {
    sample_loop: for (k = 0; k < 64; k++) {
     rand_int(rand_seed);



     int rand_value = 400 + (rand_seed
        % (800 - 400 + 1));
 ...
 ped_sub(390, 64, test_array);


 result_loop: for (i = 0; i < 64 + 2; i++) {
  if (i == 64) {
 ...
 std::cout << endl
    << "Expected pedestal change: " << (int)64/10
    << endl << "R_MIN: " << 400 << "  R_MAX: " << 800
    << "  N_SAMPLES: " << 64 << endl;


 if (390 < 400 - (int)64/10) {
  std::cout << endl
